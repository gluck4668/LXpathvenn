...
names(gene_path)
gene_pathways <- gene_path[,c("Description","pvalue","GeneRatio")]
names(gene_pathways) <- c("pathways","gene_p","GeneRatio")
...
names(protein_path)
protein_pathways <- protein_path[,c("Pathways","pvalue","Protein/Ratio")]
names(protein_pathways) <- c("pathways","protein_p","ProteinRatio")
...
if(grepl("overlapping",names(meta_path),ignore.case = T) %>% any())
  {
   ...
   meta_pathways <- meta_pathways[,c("pathway_name","P_metabolites","MetaRatio")]
